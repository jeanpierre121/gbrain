...
- [ ] **P3 — extract_facts makes no progress on this brain.** Two consecutive weekly runs on the Modal plane (2026-09-03 job 4874: 3,504 pages; 2026-09-06 job 5117: 3,551 pages) scanned every page and produced 0 inserts, 0 deletions, 0 phantom redirects with `phantomsMorePending` false, at ~45 min and one container each. The Sunday spawn and the facts-weekly healthcheck were removed on 2026-09-07 (plan exit criterion CM3 g / OV2); `extract_facts_weekly` stays as a hand-run Modal function. Re-enable when the extractor changes (new fact types, the extract_atoms drain) or when fact-bearing sources are added; first check whether `extract_facts` is meant to be idempotent on a brain whose facts were all extracted by the 2026-08 email pass (60,436 facts), in which case the weekly lane is correct to retire and the cost note in `docs/architecture/KEY_FILES.md` should say so.
