  ...
  async searchKeyword(query: string, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    if (rows.length === 0 && opts?.orFallback) {
      ...
      if (orQuery) {
        ...
        let relaxed: unknown[] | null = null;
        try {
          ({ rows: relaxed } = await this.db.transaction(async (tx) => {
        ...
        if (relaxed) return (relaxed as Record<string, unknown>[]).map((r) => ({ ...rowToSearchResult(r), keyword_relaxed: true as const }));
      }
    }
