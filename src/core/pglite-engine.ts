...
export class PGLiteEngine implements BrainEngine {
  ...
  private async applyForwardReferenceBootstrap(): Promise<void> {
    ...
    // v0.42.56.0 (v121): idx_timeline_event_page + idx_timeline_event_dedup in
    // PGLITE_SCHEMA_SQL reference event_page_id; pre-v121 brains crash without
    // the column, so bootstrap adds it before the CREATE INDEX runs. The FK +
    // indexes land via the blob and v121 (runMigrations), both idempotent.
    ...
    if (needsTimelineEventPageId) {
      ...
      // in PGLITE_SCHEMA_SQL reference it, so bootstrap adds the column before
      // the blob's CREATE INDEX runs. The FK + indexes land via the blob and
      // v121 (runMigrations), both idempotent; bootstrap only adds the column.
      await this.db.exec(`
