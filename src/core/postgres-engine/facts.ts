...
export async function findTrajectory(deps: PgFactsDeps, opts: import('../engine.ts').TrajectoryOpts): Promise<import('../engine.ts').TrajectoryPoint[]> {
    ...
    // Fork carry (run/full-facts): fail-CLOSED reader trust — an omitted `remote`
    // reads world-only; only an explicit remote:false or the stdio owner opt-in
    // (trustedFactReads) sees private facts. See ../facts/reader-trust.ts.
