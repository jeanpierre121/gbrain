...
const think: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    const result = await runThink(ctx.engine, {
      ...
      takesHoldersAllowList: ctx.takesHoldersAllowList,
      ...thinkScope,
      // Fail-closed: anything not strictly false is untrusted (CLAUDE.md invariant).
      remote: ctx.remote !== false,
    });
...
const find_trajectory: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    const points = await ctx.engine.findTrajectory({
      ...
      // Fail-closed: anything not strictly false is untrusted (CLAUDE.md invariant).
      remote: ctx.remote !== false,
      // Owner opt-in (facts.trust_local_reads), fork-local: reader-trust.ts.
...
const whoami: Operation = {
  ...
  description:
    ...
    '{transport: "stdio", scopes: []} (auth-less local MCP pipe), or ' +
