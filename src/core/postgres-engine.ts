...
export class PostgresEngine implements BrainEngine {
  ...
  private async applyForwardReferenceBootstrap(injectedConn?: postgres.Sql): Promise<void> {
    ...
    // SCHEMA_SQL reference event_page_id; pre-v121 brains crash without the
    // column, so bootstrap adds it before SCHEMA_SQL replay creates the
    // indexes. The FK + indexes land via the blob and v121 (runMigrations),
    // both idempotent.
    ...
    if (needsTimelineEventPageId) {
      ...
      // in SCHEMA_SQL reference it, so bootstrap adds the column before
      // SCHEMA_SQL replay creates the indexes. The FK + indexes land via the
      // blob and v121 (runMigrations), both idempotent; bootstrap only adds
      // the column.
      await conn.unsafe(`
