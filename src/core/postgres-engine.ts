  ...
  async searchKeyword(query: string, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    if (rows.length === 0 && opts?.orFallback) {
      ...
      if (orQuery) {
        ...
        let relaxed: Awaited<ReturnType<typeof runKeyword>> | null = null;
        try {
          relaxed = await runKeyword(orQuery, OR_FALLBACK_STATEMENT_TIMEOUT);
        } catch (err) {
        ...
        // On timeout `relaxed` stays null and the strict-AND rows win.
        if (relaxed) return relaxed.map((r) => ({ ...rowToSearchResult(r), keyword_relaxed: true as const }));
      }
    }
