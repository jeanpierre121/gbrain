  ...
  try {
    ...
    if (phases.includes('recompute_emotional_weight')) {
      ...
      if (!engine) {
        ...
        const { result, duration_ms } = await racedTimePhase('recompute_emotional_weight', () =>
    ...
    if (phases.includes('propose_takes') ||
      ...
      if (engine) {
        ...
        if (phases.includes('propose_takes')) {
          ...
          const { result, duration_ms } = await timePhase('propose_takes', () => runPhaseProposeTakes(calibrationCtx, { dryRun, repoPath: brainDir ?? undefined, deadlineAtMs: opts.deadlineAtMs ?? null, once: opts.onceForPhase === 'propose_takes' }) as Promise<PhaseResult>);
          result.duration_ms = duration_ms;
          phaseResults.push(result);
          progress.finish();
          await safeYield(opts.yieldBetweenPhases);
        ...
        if (phases.includes('grade_takes')) {
          ...
          const { result, duration_ms } = await timePhase('grade_takes', () => runPhaseGradeTakes(calibrationCtx, { dryRun, deadlineAtMs: opts.deadlineAtMs ?? null }) as Promise<PhaseResult>);
          result.duration_ms = duration_ms;
          phaseResults.push(result);
          progress.finish();
          await safeYield(opts.yieldBetweenPhases);
        ...
        if (phases.includes('calibration_profile')) {
          ...
          const { result, duration_ms } = await timePhase('calibration_profile', () => runPhaseCalibrationProfile(calibrationCtx, { dryRun, deadlineAtMs: opts.deadlineAtMs ?? null }) as Promise<PhaseResult>);
          result.duration_ms = duration_ms;
          phaseResults.push(result);
          progress.finish();
          await safeYield(opts.yieldBetweenPhases);
