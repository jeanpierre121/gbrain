...
export interface OperationContext {
   ...
   */
  remote: boolean;
  /** Fork carry (run/full-facts): owner opt-in for the local stdio pipe to read
   *  this owner's private facts (find_trajectory / recall). Set only by
   *  src/mcp/server.ts from `facts.trust_local_reads`; HTTP never sets it.
   *  See src/core/facts/reader-trust.ts. */
